...
export default function GalleryClient({ initialData }: GalleryClientProps) {
  ...
  const [selectedImage, setSelectedImage] = useState<SanityGalleryData | null>(null);
  ...
  const currentImages = filteredData.slice(startIndex, startIndex + itemsPerPage);

  if (!isMounted) return null;

  return (
    <main className="relative min-h-screen bg-slate-50 pt-32">
      <Navbar />
      <div className="container mx-auto px-6 md:px-12 mb-14">
        ...
        {filteredData.length === 0 ? (
          ...
          <FadeInStagger key={`${activeCategory}-${currentPage}-${itemsPerPage}`}>
        ...
        {totalPages > 1 && (
          <div className="mt-16 flex items-center justify-center gap-4">
            ...
            <span className="font-medium text-slate-600">
              Halaman <span className="font-bold text-red-900">{currentPage}</span> dari {totalPages}
            </span>
            <button
              onClick={() => setCurrentPage((prev) => prev + 1)}
